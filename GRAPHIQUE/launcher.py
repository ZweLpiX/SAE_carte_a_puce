...
def launch_script(script_name):
    subprocess.run(["python3", f"/home/user/Bureau/GRAPHIQUE/{script_name}"])

root = tk.Tk()
root.title("Launcher")

# Chargement et redimensionnement de l'image de café
cafe_image = Image.open("/home/user/Bureau/GRAPHIQUE/cafe.jpg")
...
# Bouton pour Rubrovitamin
button_rubro = tk.Button(root, text="Lancer Rubrovitamin", command=lambda: launch_script("Rubro_GUI.py"), bg=button_color, fg=button_text_color, font=button_font)
button_rubro.pack(fill=tk.X, pady=5)

# Bouton pour Lubiana
button_lubiana = tk.Button(root, text="Lancer Lubiana", command=lambda: launch_script("LubianaGUI.py"), bg=button_color, fg=button_text_color, font=button_font)
button_lubiana.pack(fill=tk.X, pady=5)

# Bouton pour Rodelika
button_rodelika = tk.Button(root, text="Lancer Rodelika", command=lambda: launch_script("Rodelika_GUI.py"), bg=button_color, fg=button_text_color, font=button_font)
button_rodelika.pack(fill=tk.X, pady=5)
